...
// ESM에서 현재 모듈의 디렉토리를 얻기 위한 코드
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);

// 기존 코드에서 require.resolve 사용 부분을 아래와 같이 수정
const svgPkg = resolve(
  __dirname,
  //   /Users/mj/Desktop/dev/mzui-beta/auto-cons/icons/ant-design-icons/packages/icons-svg/svg
  //   "../icons/ant-design-icons/packages/icons-svg/svg"
  "../icons-svg/svg"
);
console.log(svgPkg);
const svgPkgDir = pkgDir.sync(path.dirname(svgPkg));
console.log(svgPkgDir);
const inlineSvgDir = path.join(svgPkgDir as string, "inline-namespaced-svg");
...
function svg2base64(svgPath: string, size = 50) {
  ...
  console.log("Converted SVG to Base64:", base64);
